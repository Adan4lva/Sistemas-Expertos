...
def tokenizar(corpus):
    ...
    # Hacer una pregunta
    query = input("\nHola - Como estas? - de que te gustaría hablar?... ")
    ...
    for caracter in answer:
        print(caracter, end='', flush=True)
        time.sleep(0.025)  # Ajusta el valor para controlar la velocidad de impresión
...
def adquirir(corpus):
    ...
    # Solicitar al usuario que ingrese nuevo conocimiento
    nuevo_conocimiento = input("\n Aregar nuevo conocimiento a la BD...\n ")

    # Concatenar el nuevo conocimiento al corpus existente
    corpus += "\n" + nuevo_conocimiento
...
while True:
    opcion = input("\n 1) Hacer Nueva Pregunta\n 2) Agregar Conocimiento a la BD\n 3) Imprimir Base de Datos\n 4) Salir del Programa                          ")

    if opcion == '1':
        tokenizar(corpus)

    elif opcion == '2':
        corpus = adquirir(corpus)

    elif opcion == '3':
        for caracter in corpus:
            print(caracter, end='', flush=True)
            time.sleep(0.001)  # Ajusta el valor para controlar la velocidad de impresión
        print("\n")
